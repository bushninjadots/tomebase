...
export type ClientFactory = (config: AIProviderConfig) => { languageModel: (model: string) => any };

const ALL_CAPABILITIES: AIProviderCapabilities = {
  canExplain: true,
  canFix: true,
  canRewrite: true,
  canGenerate: true,
...
export class GenericProvider extends BaseAIProvider {
  type: AIProviderType;
  capabilities: AIProviderCapabilities = ALL_CAPABILITIES;
  config: AIProviderConfig;

  private clientFactory: ClientFactory;
  private defaultModel: string;

  constructor(config: AIProviderConfig, type: AIProviderType, clientFactory: ClientFactory, defaultModel: string) {
    super();
    this.config = config;
    this.type = type;
